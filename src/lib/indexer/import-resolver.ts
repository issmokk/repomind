...
export function resolveImport(
  ...
  const fileSet = new Set(repoFilePaths)
  ...
  if (language === 'typescript' || language === 'javascript') {
    if (source.startsWith('.')) {
      ...
      if (fileSet.has(resolved)) return { originalSource: source, resolvedPath: resolved, isExternal: false, packageName: null }
      for (const ext of TS_EXTENSIONS) {
        const candidate = resolved + ext
        if (fileSet.has(candidate)) return { originalSource: source, resolvedPath: candidate, isExternal: false, packageName: null }
      }
  ...
  if (language === 'ruby') {
    if (source.startsWith('./') || source.startsWith('../')) {
      ...
      if (fileSet.has(withRb)) return { originalSource: source, resolvedPath: withRb, isExternal: false, packageName: null }
      return { originalSource: source, resolvedPath: null, isExternal: false, packageName: null }
    }
